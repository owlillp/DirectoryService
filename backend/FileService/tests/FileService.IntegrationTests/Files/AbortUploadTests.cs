...
/// <summary>
/// Integration tests for the abort upload endpoint (POST /files/{fileId}/abort).
/// </summary>
public class AbortUploadTests(IntegrationTestsWebFactory factory) : FileServiceTestsBase(factory)
{
    private const string VIDEOS_BUCKET = "videos";

    [Fact]
    public async Task Abort_upload_of_uploading_asset_marks_it_deleted()
    {
        // arrange
        ...
        // act
        var response = await AppHttpClient.PostAsJsonAsync($"/files/{fileId}/abort", new { }, cancellationToken);
    ...
    [Fact]
    public async Task Abort_upload_of_uploaded_asset_should_fail()
    {
        // arrange
        var cancellationToken = new CancellationTokenSource().Token;
        ...
        // act
        // abort only targets assets still in UPLOADING state
        var response = await AppHttpClient.PostAsJsonAsync($"/files/{fileId}/abort", new { }, cancellationToken);
        var result = await response.HandleResponseAsync(cancellationToken);
        ...
        // the asset must remain untouched
        await AssertStatusAsync(fileId, MediaStatus.UPLOADED, cancellationToken);
    }

    [Fact]
    public async Task Abort_upload_of_non_existing_file_should_fail()
    {
        // arrange
        var cancellationToken = new CancellationTokenSource().Token;
